...
(defun make-heap ()
  "cria um heap (vector de tamanho ajustavel) "
  (make-array 100 :fill-pointer 0 :adjustable t))

(defun heap-val (heap i f-comp)
 "indica o valor de um elemento na posicao i do heap"
 (declare (fixnum i)) (funcall f-comp (aref heap i)))

(defun heap-parent (i)
 "retorna o pai da posicao i do heap"
 (declare (fixnum i)) (floor (- i 1) 2))

(defun heap-left (i)
 "retorna o filho esquerdo do pai da posicao i do heap"
 (declare (fixnum i)) (the fixnum (+ 1 i i)))

(defun heap-right (i)
 "retorna o filho direito do pai da posicao i do heap"
 (declare (fixnum i)) (the fixnum (+ 2 i i)))

(defun heapify (heap i f-comp)
  (let ((l (heap-left i))
  (r (heap-right i))
  (N (- (length heap) 1))
  smallest)
    (setf smallest (if (and (<= l N) (<= (heap-val heap l f-comp)
           (heap-val heap i f-comp)))
           l i))
    (if (and (<= r N) (<= (heap-val heap r f-comp) (heap-val heap smallest f-comp)))
  (setf smallest r))
    (when (/= smallest i)
      (rotatef (aref heap i) (aref heap smallest))
      (heapify heap smallest f-comp))))

(defun heap-extract-min (heap f-comp)
  "Pop do valor do heap (alterando-o) e utilizando a funcao f-comp para comparar"
  (let ((min (aref heap 0)))
    (setf (aref heap 0) (aref heap (- (length heap) 1)))
    (decf (fill-pointer heap))
    (heapify heap 0 f-comp)
    min))

(defun heap-insert (heap item f-comp)
  "insere um item no heap utilizando a funcao f-comp para comparar elementos"

  (vector-push-extend nil heap)
  (let ((i (- (length heap) 1))
  (val (funcall f-comp item)))
    (loop while (and (> i 0) (>= (heap-val heap (heap-parent i) f-comp) val))
...
(defun add-to (lista elem)
	"adiciona um elemento a lista de prioridades"
  (heap-insert lista elem #'(lambda (elem) (elemento-valor elem))))

(defun pop-elem (lista)
	"retira o primeiro elemento da lista de prioridades e retorna-o"
  (if (not (zerop (fill-pointer lista)))
  (heap-extract-min lista #'(lambda (elem) (elemento-valor elem)))))

(defun peek-elem (lista)
	"ve o valor minimo da lista de prioridades"
  (aref lista 0))

(defun procura-A* (problema f-heuristica)
	"realiza a procura A* num dado problema com a heuristica f-heuristica e retorna uma lista de accoes que o resolvem"
    (let (  (estado-inicial (problema-estado-inicial problema))
            (f-solucao          (problema-solucao problema))
            (f-accoes           (problema-accoes problema))
            (f-resultado        (problema-resultado problema))
      ...
      ; enquanto a lista de prioridades tiver elementos:
      (loop while (not (zerop (fill-pointer lista-prioridade))) do
            (setf top-elem (pop-elem lista-prioridade))
            ; se for solucao entao retorna a lista de accoes do elemento
...
(defun heuristica-geral (estado)
  ...
  (let (( valor0 1000) ( valor1 50) ( valor2 100) ( valor3 150))
